...
class OccupancyRateController extends Controller
{
    ...
    public function room(Request $request)
    {
        ...
        $occupancy_by_year_by_room->setColors(["#693c72", "#c15050", "#d97642", "#d49d42", "#9b3675"]);

        $categories = \App\Models\Room::distinct()->get(["category"])->pluck(['category']);
        
        $categories->each(function($category) use($occupancy_by_month_by_room, $occupancy_by_quarter_by_room, $month_year, $quarter_year, $occupancy_by_year_by_room, $start_year, $end_year, $numberFloat)
        {
            $occupancy_by_month_by_room->addData($category, $this->occupancyRate->byMonthByRoom($category, $month_year)->pluck('occupancy_rate')->map($numberFloat)->toArray());
            $occupancy_by_quarter_by_room->addData($category, $this->occupancyRate->byQuarterByRoom($category, $quarter_year)->pluck('occupancy_rate')->map($numberFloat)->toArray());
            $occupancy_by_year_by_room->addData($category, $this->occupancyRate->byYearByRoom($category, $start_year, $end_year)->pluck('occupancy_rate')->map($numberFloat)->toArray());
